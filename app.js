...
let args = minimist(process.argv.slice(2), {
	...
	default: {
		url : 'https://proxystatc.herokuapp.com/ufax', // Url to navigate
		proxy : '', // Proxy address
		port : '', // Proxy port
		user: '', // Proxy username
		pass: '', // Proxy pass
...
const venenoTrafficBot = async id => {
	...
	let blacklist = ['https://www.facebook.com/','https://instagram.com/', 'https://www.superlivescore.com/','https://apps.apple.com/','https://twitter.com/'];
	...
	if (url == '') {
		...
		console.log('-----------------------------------------------------');
		console.log('ERROR! Please enter a username of the proxy server.');
		console.log('-----------------------------------------------------');
		process.exit(1);
	} else if (user != '' && pass == '') {
		console.log('-----------------------------------------------------');
		console.log('ERROR! Please enter a password of the proxy server.');
		console.log('-----------------------------------------------------');
		process.exit(1);
	}
	...
	let utm_medium = ['banner', 'cpc'];
	let utm_campaign = ['sale', 'bet'];
	let utm_term = ['ufatop1','แทงบอล','ผลบอล', 'พนันออนไลน์', 'แนวทาง', 'วิธีเล่นบอลสเต็ป อย่างไรให้รวย', 'สูตรบอลสเต็ปวันนี้', 'สูตรบอลชุด', 'ทีเด็ดบอลสเต็ป', 'แจกสูตรบอลชุด', 'วิธีดูราคาบอล', 'วิธีดูราคาบอลที่ถูกต้อง','สอนดูตารางบอล','วิธีดูราคาบอลสูงต่ำ','เว็บแทงบอล','แทงบอลสเต็ป','วิธี แทงบอล','เว็บพนันบอล','แทงบอล ไม่มีขั้นต่ำ','แทงบอล ufabet','แทงบอล ฟรีเครดิต','แทงบอล ยัง ไง','พนันบอลดียังไง','ผลบอลเมื่อคืนนี้','ผลบอลย้อนหลัง','โปรแกรมบอล','เช็คผลบอลล่าสุด'];
